...
export const updateVisibleBySearchFlattedOptions = (flattedOptions: FlattedOptionType[], searchText: string) => {
  ...
  const optionsWithSearchText = getSearchedFlattedOptions(flattedOptions, normalizedSearchText);
...
const getSearchedFlattedOptions = (flattedOptions: FlattedOptionType[], searchText: string) => {
  ...
  return flattedOptions.reduce((acc, curr) => {
    // Also normalize the name of the current option for the comparison
    const isSearched = removeDiacritics(curr.name).toLowerCase().includes(normalizedSearchText);
